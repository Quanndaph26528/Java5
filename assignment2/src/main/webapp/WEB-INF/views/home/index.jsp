      ...
      <form action="/" method="GET">
        ...
        <div class="brand-search-info mt-3">
          ...
          <c:forEach var="c" items="${categoryList}">
            <div class="mt-1">
              <input name="categoryId" type="radio" value="${c.id}" <c:if test="${param.categoryId==c.id}">checked</c:if> />
              <span>${c.name}</span>
            </div>
          </c:forEach>
        </div>
        ...
        <div class="price-search-info mt-3">
          ...
          <c:forEach var="pr" items="${priceRangeList}">
            <div class="mt-2">
              <input type="radio" name="priceRangeId" value="${pr.id}" <c:if test="${param.priceRangeId==pr.id}">checked</c:if> />
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination">
          <c:if test="${currentPage > 0}">
            <li class="page-item">
              <a class="page-link" href="?categoryId=${categoryId}&amp;keyword=${param.keyword}&amp;priceRangeId=${param.priceRangeId}&amp;page=${currentPage - 1}">&laquo;</a>
            </li>
          </c:if>
          <c:forEach var="page" begin="0" end="${totalPages}">
            <li class="page-item <c:if test='${page == currentPage}'>active</c:if>">
              <a class="page-link" href="?categoryId=${categoryId}&amp;keyword=${param.keyword}&amp;priceRangeId=${param.priceRangeId}&amp;page=${page}">${page}</a>
            </li>
          </c:forEach>
          <c:if test="${currentPage < totalPages}">
            <li class="page-item">
              <a class="page-link" href="?categoryId=${categoryId}&amp;keyword=${param.keyword}&amp;priceRangeId=${param.priceRangeId}&amp;page=${currentPage + 1}">&raquo;</a>
            </li>
          </c:if>
        </ul>
        <span>Hiển thị ${startProduct}-${endProduct} trên tổng số ${totalProducts} sản phẩm</span>
      </nav>
    </div>
  </div>
</div>
